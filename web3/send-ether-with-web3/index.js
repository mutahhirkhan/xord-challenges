...
const main = async () => {
    try {
        ...
        let destinationWallet = web3.eth.getBalance(process.env.DESTINATION_WALLET_ADDRESS).toNumber();

        let myBalance = web3.fromWei(myBalanceWei, "ether");
        let destinationWalletBalance = web3.fromWei(destinationWallet, "ether");

        log(`Your wallet balance is currently ${myBalance} ETH`.green);
        log(`destination wallet balance is currently ${destinationWalletBalance} ETH`.green);
        ...
        let myBalanceUpadted = web3.fromWei(myBalanceWei, "ether");
        let destinationWalletBalanceUpadted = web3.fromWei(destinationWallet, "ether");

        log(`Your wallet balance is currently ${myBalanceUpadted} ETH`.green);
        log(`destination wallet balance is currently ${destinationWalletBalanceUpadted} ETH`.green);
        process.exit();
    } catch (error) {
        console.log(error);
    }
